...
app.get("/challenge/:id", async (req, res) => {
  ...
  try {
    ...
    if (chal) {
      ...
      res
        .status(400)
        .send(`Challenge with id \"${req.query.id}\" could not be found`);
    }
  } catch (error) {
    console.log(error);
    res.status(500).send({
...
app.post("/challenge", async (req, res) => {
  ...
  if (
    ...
    res
      .status(400)
      .send("Bad request: Missing name, points, session or course");
    return;
  }
  try {
    // Connect to the database
...
app.delete("/challenges/:id", async (req, res) => {
  try {
    ...
    if (result.deletedCount === 1) {
      res
        .status(200)
        .send(`Challenge with id "${req.query.id}" successfully deleted.`);
    } else {
      res
        .status(404)
